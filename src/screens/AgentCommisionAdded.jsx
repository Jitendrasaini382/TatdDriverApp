...
import {AGENT_WALLET_DETAILS, GET_AGENT_WALLET} from '../apis/Apis';
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: AppColors.white}}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.detailsRow}>
          <Text style={styles.detailText}>Your {walletData?.commission_layer}</Text>
          <Text style={styles.detailText}>{walletData?.timestamp}</Text>
        </View>

        <View>
            {
                walletData?.customer_name &&<Text style={styles.agentName}>{walletData?.customer_name}</Text>
            }
          {
            walletData?.invoice_value == "Driver Registration" ? <Text style={styles.invoiceValue}>
            {/* Invoice Value {''} */}
            <Text>
              {/* <Icon name="rupee" size={15} color={AppColors.black} /> */}
              {walletData?.invoice_value}
            </Text>
          </Text>: <Text style={styles.invoiceValue}>
            Invoice Value {''}
            <Text>
              <Icon name="rupee" size={15} color={AppColors.black} />
              {walletData?.invoice_value}
